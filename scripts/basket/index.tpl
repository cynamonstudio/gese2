                        ...
                        <div class="box unibox" id="box_basketlist">
                            ...
                            <div class="innerbox">
                                <form action="{route key='basket'}" method="post">
                                                ...
                                                {if !$basket_product->isChild()}
                                                    <tr data-id="{$basket_product->getIdentifier()}" data-product-id="{$basket_product->product->product->product_id}" data-category="{$basket_product->product->defaultCategory->translation->name|escape}" {if $basket_product->product->product->producer_id}data-producer="{$basket_product->product->producer->manufacturer->name|escape}"{/if}>
                                                        ...
                                                        <td class="quantity">
                                                            ...
                                                            <span class="shaded_inputwrap{if true == $quantity_error.$id} error{/if}"><input name="quantity_{$id}" value="{float precision=$QUANTITY_PRECISION value=$quantity.$id trim=true noformat=true}" type="{if $basket_product->product->unit->unit->floating_point == 0}number{else}text{/if}" class="short center" aria-label="{translate key='quantity'}" /></span>
                                                        ...
                                                        <td class="actions">
                                                            <a href="{route key='basketRemove' basketId=$basket_product->getIdentifier()}" title="{translate key='Remove product from cart'}" class="prodremove" aria-label="{translate key='Remove'} {$basket_product->product->translation->name|escape}">
                                                                <img src="{baseDir}/libraries/images/1px.gif" alt="" class="px1">
                                    ...
                                    {if $inpostPayWidgetSettings.isEnabled}
                                        {assign var=displayInpostShipping value=false}
                                        {foreach from=$shippings item=shipping name=list}
                                            {if $shipping->getIdentifier()|in_array:$inpostPayWidgetSettings.mappedShippingMethodsId}
                                                {assign var=displayInpostShipping value=true}
                                            {/if}
                                        {/foreach}
                                    {/if}

                                    {if $inpostPayWidgetSettings.isEnabled && $inpostPayWidgetSettings.showInBasket && $displayInpostShipping}<!--new-inpost-izi-button-->
                                        {if $inpostPayWidgetSettings.merchantClientId}
                                            <inpost-izi-button
                                                data-v2
                                                variation="
                                                    {if $inpostPayWidgetSettings.buttonVariant == 'secondary'}secondary {else}primary {/if}
                                                    {if $inpostPayWidgetSettings.isDarkMode}dark {/if} {$inpostPayWidgetSettings.frameStyle}"
                                                binding_place="BASKET_SUMMARY">
                                            </inpost-izi-button>
                                        {else}
                                            <inpost-izi-button
                                                data-v1
                                                class="izi-button-basket"
                                                name=""
                                                masked_phone_number=""
                                                language="{lang key='short'}"
                                                variant="{$inpostPayWidgetSettings.buttonVariant}"
                                                basket="true"
                                                dark_mode="{if $inpostPayWidgetSettings.isDarkMode}true{else}false{/if}"
                                                count="0"
                                                frame_style="{$inpostPayWidgetSettings.frameStyle}"
                                                binding_place="BASKET_SUMMARY"
                                            >
                                            </inpost-izi-button>
                                        {/if}
                                    ...
                                    <div id="cart-options">
                                        <div class="delivery-container">
                                            ...
                                            <div {if $shipping_extra_step}class="none"{/if}>
                                                ...
                                                {foreach from=$shippings item=shipping name=list}
                                                    {assign var='shippingDescId' value="shipping-desc-`$smarty.foreach.list.index`"}
                                                        ...
                                                        <span class="name">
                                                            <span class="radio-wrap">
                                                                <input
                                                                    type="radio"
                                                                    name="shipping_id"
                                                                    id="shipping_{$shipping->getIdentifier()}"
                                                                    value="{$shipping->getIdentifier()}"
                                                                    {if $shipping_id == (int) $shipping->getIdentifier()}checked="checked" {/if}
                                                                    aria-describedby="{$shippingDescId}"
                                                                />
                                                            ...
                                                            <span class="description" id="{$shippingDescId}">
                                        ...
                                        <div class="payment-container{if $shipping_extra_step} none{/if}">
                                            ...
                                            {foreach from=$payments item=payment name=list}
                                                {assign var='paymentDescId' value="payment-desc-`$smarty.foreach.list.index`"}
                                                    ...
                                                    <span class="name">
                                                        <span class="radio-wrap">
                                                            <input
                                                                type="radio"
                                                                name="payment_id"
                                                                id="payment_{$payment->getIdentifier()}"
                                                                value="{$payment->getIdentifier()}"
                                                                {if $payment_id == (int) $payment->getIdentifier()}checked="checked" {/if}
                                                                {if $payment->translation->description}aria-describedby="{$paymentDescId}"{/if}
                                                            />
                                                        ...
                                                        <span class="description" id="{$paymentDescId}">
                                                        ...
                                                        <div class="promocode">
                                                            ...
                                                            <span class="input">
                                                                <div class="shaded_inputwrap{if true == $promocode_error} error{/if}">
                                                                    <input type="text" value="{$promocode|escape}" name="promocode" aria-label="{translate key='Coupon code'}">
                                                                </div>
                                                            </span>
                                                            <span class="action">
