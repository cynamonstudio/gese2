                                    ...
                                    <table class="address-handler">
                                        <tbody>
                                            {foreach from=$table item=tr}
                                                ...
                                                {if $tr.name|escape == 'name'}
                                                    {assign var="autocomplete" value="given-name"}
                                                {elseif $tr.name|escape == 'surname'}
                                                    {assign var="autocomplete" value="family-name"}
                                                {elseif $tr.name|escape == 'phone'}
                                                    {assign var="autocomplete" value="tel"}
                                                {elseif $tr.name|escape == 'coname'}
                                                    {assign var="autocomplete" value="organization"}
                                                {elseif $tr.name|escape == 'other_address'}
                                                    {assign var="autocomplete" value="street-address"}
                                                {elseif $tr.name|escape == 'zip'}
                                                    {assign var="autocomplete" value="postal-code"}
                                                {elseif $tr.name|escape == 'city'}
                                                    {assign var="autocomplete" value="address-level2"}
                                                {elseif $tr.name|escape == 'country'}
                                                    {assign var="autocomplete" value="country-name"}
                                                {/if} 
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="input">
                                                        {if 'select' == $tr.type}
                                                            <select class="{if $tr.error} error{/if}" name="{$tr.name|escape}" id="input_{$tr.name|escape}" autocomplete="{$autocomplete}">
                                                        ...
                                                        {else}
                                                            <div class="shaded_inputwrap{if $tr.error} error{/if}">
                                                                <input 
                                                                    {if $tr.name == 'zip'}data-mask="__-___" data-pattern="{literal}^(\d{2})(-)(\d{3})${/literal}" data-valid-pattern="00-000"{/if}  
                                                                    type="{$tr.type|escape}" name="{$tr.name|escape}" value="{$tr.value|escape}" size="25" id="input_{$tr.name|escape}" autocomplete="{$autocomplete}" 
                                                                    {if true == $tr.obligatory} aria-required="true"{/if} {if $tr.error}aria-invalid="true"{/if}
                                                                    {if $tr.hint or $tr.error}aria-describedby="{if $tr.hint }hint_{$tr.name|escape} {/if}{if $tr.error}error_{$tr.name|escape}{/if}"{/if}>
                                                            </div>
                                                        {/if}
                                                    </td>

                                                    {if $tr.rowspan > 0}
                                                        <td class="{if $tr.hint}hint{/if}"{if $tr.rowspan > 1} rowspan="{$tr.rowspan|escape}"{/if} id="hint_{$tr.name|escape}">
                                                ...
                                                {if $tr.error}
                                                    <tr>
                                                        <td />
                                                        <td class="error">
                                                            <ul class="input_error" id="error_{$tr.name|escape}">
