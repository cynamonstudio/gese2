                            ...
                            <form action="{route key='panelEdit'}" method="post">
                                <fieldset>
                                    ...
                                    <div class="shaded_inputwrap">
                                        <input type="text" name="mail" id="mail_input" value="{$data.mail|escape}" size="30" class="{if $data_error.mail} error{/if}" {if $data_error.mail}aria-invalid="true" aria-describedby="error_mail"{/if} aria-required="true" autocomplete="email">
                                    </div>
                                    {if $data_error.mail}
                                        <ul class="input_error error" id="error_mail">
                                    ...
                                    <div class="shaded_inputwrap">
                                        <input type="text" name="name" id="name_input" value="{$data.name|escape}" size="30" class="{if $data_error.name} error{/if}" {if $data_error.name}aria-invalid="true" aria-describedby="error_name"{/if} autocomplete="given-name">
                                    </div>
                                    {if $data_error.name}
                                        <ul class="input_error error" id="error_name">
                                    ...
                                    <div class="shaded_inputwrap">
                                        <input type="text" name="surname" id="surname_input" value="{$data.surname|escape}" size="30" class="{if $data_error.surname} error{/if}" {if $data_error.surname}aria-invalid="true" aria-describedby="error_surname"{/if} autocomplete="family-name">
                                    </div>
                                    {if $data_error.surname}
                                        <ul class="input_error error" id="error_surname">
                                    ...
                                    <div class="shaded_inputwrap">
                                        <span class="checkbox-wrap">
                                            <input type="checkbox" name="newsletter" id="newsletter_input" value="1" {if 1 == $data.newsletter} checked{/if} {if $data_error.newsletter}aria-invalid="true" aria-describedby="error_newsletter"{/if}>
                                    ...
                                    {if $data_error.newsletter}
                                        <ul class="input_error" id="error_newsletter">
                                        ...
                                        {foreach from=$additional_fields item=field}
                                            ...
                                            {/if}
                                                ...
                                                {if $field->field->type == constant('Logic_AdditionalField::TYPE_HIDDEN')}
                                                    {additionalField field=$field name=$name value=$data.$name|escape error=$additional_error.$name}
                                                ...
                                                {else}
                                                    <div class="input">
                                                        ...
                                                        {if $field->field->type == constant('Logic_AdditionalField::TYPE_CHECKBOX')}
                                                            <span class="checkbox-wrap">
                                                                {additionalField field=$field name=$name value=$data.$name editable=true error=$additional_error.$name}
                                                        ...
                                                        {elseif $field->field->type == constant('Logic_AdditionalField::TYPE_SELECT')}
                                                            {additionalField field=$field name=$name value=$data.$name|escape editable=true error=$additional_error.$name}
                                                        {else}
                                                            <span class="shaded_inputwrap{if $additional_error.$name} error{/if}">
                                                                {additionalField field=$field name=$name value=$data.$name|escape editable=true size=25 error=$additional_error.$name}
                                                            </span>
                                                        {/if}
                                                    </div>
                                                {/if}
                                            ...
                                            {if $additional_error.$name}
                                                <div class="error">
                                                    {formErrors errors=$additional_error.$name class="input_error" name=$name}
                                                </div>
                                            {/if}
                                        {/foreach}
                                    {/if}
