...
<body{if $body_id} id="{$body_id|escape}"{/if}{if $body_class} class="{$body_class|escape}"{/if}>
                                        ...
                                        <div class=" f-grid-6">
                                            ...
                                            <div class="bottomborder row" itemprop="offers" itemscope itemtype="{$schema}://schema.org/Offer">
                                                <div class="basket">
                                                    ...
                                                    {elseif $showprices}
                                                        <!-- mod -->
                                                        {if $user_logged && $user->user->isInGroup(1)}
                                                    ...
                                                    <form class="form-basket{if false == $product->canBuyStock() && ($enable_availability_notifier && $product->isEnabledNotifier()) == false} none{/if}{if $loyalty_exchange} loyaltyexchange{/if}" method="post" action="{route key=$basketAddRoute stockId='post'}" enctype="multipart/form-data">
                                                        ...
                                                        {if $product->product->group_id && count($options)}
                                                            {assign var="stocks_flag" value="true"}
                                                            <div class="stocks">
                                                                {foreach from=$options item=option}
                                                                    ...
                                                                    <div class="f-row">
                                                                        ...
                                                                        {/if}
                                                                        <div class="stock-options f-grid-6{if $option.type == 'select'} no-label{/if}">
                                                                            {if  $option.type == 'file'}
                                                                            ...
                                                                            {elseif  $option.type == 'select' && $stocks_flag == "true"}
                                                                                {if $enablebasket}
                                                                                    {assign var="stocks_flag" value="false"}
                                                                                    <table class="stocks_table">
                                                                                        <tbody>
                                                                                            {assign var=stocks value=null}
                                                                                            {assign var=sortedOptions value=null}
                                                                                            {foreach from=$product->optionsStock item=stock}
                                                                                                {assign var=temp4 value=$stock->getIdentifier()}
                                                                                                {append var=stocks value=$stock index=$stock->getIdentifier()}
                                                                                                {foreach from=$stock->values item=sv}
                                                                                                    {if $sv->option->translation->name == 'Długość'}
                                                                                                        {assign var=ovalueId value=$sv->ovalue->getIdentifier()}
                                                                                                        {assign var=prevStocks value=$sortedOptions.$ovalueId}
                                                                                                        {append var=prevStocks value=$stock->getIdentifier()}
                                                                                                        {append var=sortedOptions value=$prevStocks index=$ovalueId}
                                                                                                    {/if}
                                                                                                {/foreach}
                                                                                            {/foreach}
                                                                                            {if $sortedOptions|@count > 0}
                                                                                                {foreach from=$sortedOptions item=sortedStocks}
                                                                                                    {foreach from=$sortedStocks item=stockId key=key}
                                                                                                        {assign var=stock value=$stocks.$stockId}
                                                                                                        {if $stock->stock->stock !== "0"}
                                                                                                            {if $key == 0}
                                                                                                                {foreach from=$stock->values item=sv}
                                                                                                                    {if $sv->option->translation->name == 'Długość'}
                                                                                                                        <tr><th colspan="5">{$sv->ovalue->translation->value}</th></tr>
                                                                                                                    {/if}
                                                                                                                {/foreach}
                                                                                                            {/if}
                                                                                                            <tr data-id="{$stock->getIdentifier()}">
                                                                                                                <td class="cell_image">
                                                                                                                    <img class="" src="{imageUrl type='productGfx' width=$skin_settings->img->small height=$skin_settings->img->small image=$stock->gfx->unic_name}" alt="{$stock->gfx->name|escape}">
                                                                                                                </td>
                                                                                                                <td class="cell_options">
                                                                                                                    {assign var=options value=null}
                                                                                                                    {foreach from=$stock->values item=sv}
                                                                                                                        {append var=options value=$sv->ovalue->getIdentifier() index=$sv->option->getIdentifier()}
                                                                                                                        {if $sv->option->translation->name == "Gramatura"}
                                                                                                                            {assign var="mod_weight" value=$sv->ovalue->translation->value}
                                                                                                                        {/if}

                                                                                                                        <div>{$sv->option->translation->name}: {$sv->ovalue->translation->value}</div>
                                                                                                                    {/foreach}
                                                                                                                    <input type="hidden" class="options" value='{$options|@json_encode}'>
                                                                                                                </td>
                                                                                                                <td class="cell_price">
                                                                                                                    {if $user_logged && $user->user->isInGroup(1)}
                                                                                                                        {if $stock->getSpecialOfferPrice(true) != $stock->getPrice(true)}
                                                                                                                            <div class="netto">{currency value=$stock->getSpecialOfferPrice(true)}</div>
                                                                                                                            <del>{currency value=$stock->getPrice(true)}</del>
                                                                                                                        {else}
                                                                                                                            <div class="netto">{currency value=$stock->getPrice(true)}</div>
                                                                                                                        {/if}
                                                                                                                    {else}
                                                                                                                        {if $stock->getSpecialOfferPrice() != $stock->getPrice()}
                                                                                                                            <div>{currency value=$stock->getSpecialOfferPrice()}</div>
                                                                                                                            <del>{currency value=$stock->getPrice()}</del>
                                                                                                                        {else}
                                                                                                                            <div>{currency value=$stock->getPrice()}</div>
                                                                                                                        {/if}
                                                                                                                    {/if}
                                                                                                                </td>
                                                                                                                <td class="cell_quantity">
                                                                                                                    <div class="mx_ilosc">
                                                                                                                        <i class="fa fa-minus mx_minus" aria-hidden="true"></i>
                                                                                                                        <input type="number" name="" value="0" min="0" max="{$stock->stock->stock}">
                                                                                                                        <i class="fa fa-plus mx_plus" aria-hidden="true"></i>
                                                                                                                    </div>
                                                                                                                    <div>
                                                                                                                        dostępne:
                                                                                                                        <span class="mx_avail">{$stock->stock->stock}</span>
                                                                                                                    </div>
                                                                                                                </td>
                                                                                                                <td class="cell_button {if !$stock->availability->availability->can_buy}disabled{/if}">
                                                                                                                    <button type="button" class="btn {if $stock->availability->availability->can_buy}btn-red{/if}">dodaj</button>
                                                                                                                </td>
                                                                                                            </tr>
                                                                                                        {/if}
                                                                                                    {/foreach}
                                                                                                {/foreach}
                                                                                            {elseif $product->optionsStock|@count > 0}
                                                                                                <tr class="index">
                                                                                                    <td class="cell_options">
                                                                                                        {assign var=options value=null}
                                                                                                        {foreach from=$stock->values item=sv}
                                                                                                            {append var=options value=$sv->ovalue->getIdentifier() index=$sv->option->getIdentifier()}
                                                                                                            <div>{$sv->option->translation->name}</div>
                                                                                                        {/foreach}
                                                                                                    </td>
                                                                                                    <td class="cell_available">
                                                                                                        <span>{translate key="Na stanie"}</span>
                                                                                                    </td>
                                                                                                    <td class="cell_price">
                                                                                                        {if $user_logged && $user->user->isInGroup(1)}
                                                                                                            <span>{translate key="Cena netto"}</span>
                                                                                                        {else}
                                                                                                            <span>{translate key="Cena"}</span>
                                                                                                        {/if}
                                                                                                    </td>
                                                                                                    <td class="cell_quantity">
                                                                                                        <span>{translate key="Ilość"}</span>
                                                                                                    </td>
                                                                                                </tr>
                                                                                                {foreach from=$product->optionsStock item=stock}
                                                                                                    {if $stock->stock->stock !== "0"}
                                                                                                        <tr data-id="{$stock->getIdentifier()}" {if $stock->stock->stock == "0"}class='no-stock'{/if}>
                                                                                                            <td class="cell_options">
                                                                                                                {assign var=options value=null}
                                                                                                                {foreach from=$stock->values item=sv}
                                                                                                                    {append var=options value=$sv->ovalue->getIdentifier() index=$sv->option->getIdentifier()}
                                                                                                                    {if $sv->option->translation->name == "Gramatura"}
                                                                                                                        {assign var="mod_weight" value=$sv->ovalue->translation->value}
                                                                                                                    {/if}
                                                                                                                    <div>{$sv->ovalue->translation->value}</div>
                                                                                                                {/foreach}
                                                                                                                <input type="hidden" class="options" value='{$options|@json_encode}'>
                                                                                                            </td>
                                                                                                            <td class="cell_available">
                                                                                                            ...
                                                                                                            <td class="cell_price">
                                                                                                                {if $user_logged && $user->user->isInGroup(1)}
                                                                                                                        {if $stock->getSpecialOfferPrice(true) != $stock->getPrice(true)}
                                                                                                                            <div>{currency value=$stock->getSpecialOfferPrice(true)}</div>
                                                                                                                            <del>{currency value=$stock->getPrice(true)}</del>
                                                                                                                        {else}
                                                                                                                            <div>{currency value=$stock->getPrice(true)}</div>
                                                                                                                        {/if}
                                                                                                                    {else}
                                                                                                                        {if $stock->getSpecialOfferPrice() != $stock->getPrice()}
                                                                                                                            <div>{currency value=$stock->getSpecialOfferPrice()}</div>
                                                                                                    ...
                                                                                                    <div class="stock-options f-grid-6">
                                                                                                        {if  $option.type == 'select'}
                                                                                                            <div class="option_{$option.type|escape}{if 1 == $option.stock} option_truestock{/if}{if 1 == $option.required} option_required{/if}">
                                                                                                                ...
                                                                                                                {/if}
                                                                                                                {foreach from=$option.values item=value}
                                                                                                                    <option value="{$value.id|escape}" data-option-id="{$option.id|escape}" class="mod-order">{$value.name|escape}</option>
                                                                                                                {/foreach}
                                                                                                                </select>
                                                                                                            </div>
                                                                                                        {/if}
                                                                                ...
                                                                                <div class="options-overflow">
                                                                                    <p>Przekroczona dostępna ilość sztuk!</p>
                                                                                </div>

                                                                                <div class="options-value">
                                                                                    <b>{translate key="Wartość"} <span id="value">0,00 zł</span></b>
                                                                                </div>
                                                                            {/if}
                                                                        </div>
                                                                    </div>
    ...
    jQuery(function($) {
        /* //mod - maxsote - multiwybor wariantow */

        function calcValue()
        {
            $(".options-value span#value").empty();

            let currentValue = 0;
            $(".stocks table tr:not(.index)").each(function()
            {
                let price = parseFloat($(this).find(".cell_price div").text().replace(",", ".").replace(" zł", "").replace(/\s/g, ""));
                let quantity = parseInt($(this).find('.cell_quantity input').val());
                currentValue += price * quantity;

            });
            $(".options-value span#value").text((currentValue).toFixed(2).replace(".", ",") + " zł");
        }
        ...
        /* //mod - maxsote - multiwybor wariantow */
            ...
            {
                $("fieldset.addtobasket-container .quantity_wrap").hide().next().css("width", "100%");
            }

            let options_weight = [];
            $("table.stocks_table tr:not(.index)").each(function()
            {
                let option_id = $(this).find(".cell_options input.options").val();
                let option_weight = parseFloat($(this).find(".cell_options > div").text());

                options_weight[option_weight] = option_id;
            });
            
            var keys = Object.keys(options_weight).sort(function(a, b)
            {
            ...
            {
                options_order[options_weight[keys[order]]] = order; 
            }
            ...
            $("table.stocks_table tr:not(.index)").each(function()
            {
                ...
                let id = currentRow.find(".cell_options input.options").val();
                console.log(id);
                let order = options_order[id];
                console.log(order);
                currentRow.css('order', order);

                currentRow.find(".mx_ilosc i.fa").on('click', function()
                {
                    let currentVal = parseInt(currentRow.find(".mx_ilosc input").val());
                    if ($(this).hasClass('mx_minus'))
                    {
                        currentVal--;
                    }
                    else if ($(this).hasClass('mx_plus'))
                    {
                        currentVal++;
                    }

                    currentRow.find(".mx_ilosc input").val(currentVal).change();
                });

                currentRow.find(".mx_ilosc input").on('change', function()
                {
                    calcValue();
                });
            });

            $("fieldset.addtobasket-container button.addtobasket").on('click', function(e)
            {
                if ($("table.stocks_table").length)
                {
                    e.preventDefault();
                    $(".stocks table tr:not(.index)").each(function()
                    {
                        let currentOption = $(this);
                        let stock_id = currentOption.data('id');
                        let quantity = currentOption.find(".mx_ilosc input").val();
                        let options = JSON.parse(currentOption.find(".options").val());

                        if (quantity > 0)
                        {
                            frontAPI.addToBasket(function(response)
                            {
                                currentOption.find(".mx_ilosc input").val(0).change();
                                refreshBasket();
                            },
                            {
                                data: {
        ...
        function refreshBasket()
        {
            frontAPI.getBasketInfo(function(basket)
            {
                ...
                {
                    let product = basket.products[i];
                    html += '<li>';
                    html += '   <img src="/userdata/gfx/'+product.image+'.jpg"/>';
                    html += '   <span class="mod_basket_prod_name product-name">';
                    html += '       '+product.name;
                    html += '       <span class="product-variant">'+product.variant+'</span>';
                    html += '   </span>';
                    html += '   <span class="product-info">';
                    html += '       <span class="product-amount">';
                    html += '           '+product.quantity;
                    html += '       </span>';
                    html += '       <span class="product_price"> x '+product.price+'</span>';
                    html += '   </span>';
                    html += '   <span ';
                    html += '       type="button" ';
                    html += '       class="remove-product" ';
                    html += '       data-product-basket-id="'+product.id+'" ';
                    html += '       data-product-id="'+product.product_id+'" ';
                    html += '   >';
                    html += '       <a href="#">Usuń</a>';
                    html += '   </span>';
                    html += '</li>';
                ...
                $('.price-total').html('Suma: '+basket.basket.sum_no_shipping);
                if($('div.basket.right > a > span > b > b.sum').length == 0){
                    $('div.basket.right > a > span > b').html('<b class="sum">0,00&nbsp;zł</b> <b class="count">(<span>0</span>)</b>');
                }
                $('div.basket.right > a > span > b > b.sum').html(basket.basket.sum_no_shipping);
                $('div.basket.right > a > span > b > b.count > span').text(sumQuantity);
                $('.remove-product').unbind().on('click.remove', function()
                {
                    var basket_product_id = $(this).data('product-basket-id');
                    frontAPI.removeFromBasket(function(removeInfo) {
                        refreshBasket();
                    }, {
        ...
        /* //endmod - maxsote - multiwybor wariantow */
    </script>
    {/literal}
