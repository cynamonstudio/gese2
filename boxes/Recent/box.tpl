{if count($boxNs->$box_id->list)}
        <div class="box{if 1 == $boxNs->$box_id->format} slider{if 1 == $boxNs->$box_id->automove} slider_automove{/if}{/if}" id="box_recent">
            <div class="boxhead">
                <span><img src="{baseDir}/libraries/images/1px.gif" alt="" class="px1" aria-hidden="true">{$boxNs->$box_id->title|escape}</span>
            </div>
            <div class="innerbox">
                ...
                <div data-product-id="{$special_product->product->product_id}" data-category="{$special_product->defaultCategory->translation->name|escape}" {if $special_product->product->producer_id}data-producer="{$special_product->producer->manufacturer->name|escape}"{/if} class="product row center{if $special_product->readVisibilityConfigListItemGrayFlag()} product_inactive{/if}">
                    <a
                        href="{route function='product' key=$special_product->getIdentifier() productName=$special_product->translation->name productId=$special_product->getIdentifier()}"
                        title="{$special_product->translation->name|escape}"
                        class="row"
                        aria-label="{translate key='Go to the product'} {$special_product->translation->name|escape}"
                    >
                            ...
                            {if $enablebasket && $special_product->canBuyStock()}
                                <fieldset>
                                    ...
                                    {if !$special_product->isBundle()}
                                        <button class="addtobasket btn btn-red" type="submit" aria-label="{if $loyalty_exchange}{translate key="Exchange"}{else}{translate key="Add to cart"}{/if} {$special_product->translation->name|escape}">
